...
public final class EnchantmentsWindow extends FastInv {
    ...
    private static final NamespacedKey ENCHANTMENT_SLOTS = new NamespacedKey(TheAnnihilation.getInstance(), "enchantment_slots");
    private static final NamespacedKey BUILTIN_ENCHANTMENTS = new NamespacedKey(TheAnnihilation.getInstance(), "builtin_enchantments");
                    ...
                    if (updateFlags < 0) {
                        ...
                        for (int i = 0; i < enchantments.length; i++) {
                            if ((updateFlags & 1 << i) > 0) {
                                final int slot = i;
                                Bukkit.getScheduler().runTask(TheAnnihilation.getInstance(), () -> updateEnchantment(slot));
                            }
                        }
                    }
                }
        ...
        for (final Map.Entry<Integer, ItemStack> entry : event.getNewItems().entrySet()) {
            ...
            if (event.getView().convertSlot(slot) == slot) {
                if (checkCompatible(entry.getValue())) {
                    Bukkit.getScheduler().runTask(TheAnnihilation.getInstance(), () -> updateEnchantment(enchantmentSlot(slot)));
                } else {
    ...
    protected void onClose(InventoryCloseEvent event) {
        if (source != null) {
            Bukkit.getScheduler().runTask(TheAnnihilation.getInstance(), () -> event.getPlayer().openInventory(source));
        }
    }
