package me.paperxiang.theannihilation.i18n;
import java.io.File;
...
public final class I18n {
    ...
    public static void init() {
        loadI18ns(TheAnnihilation.getInstance());
    }
    public static void writeDefaultConfigurableI18ns(JavaPlugin plugin) {
        ...
        try (final InputStream stream = plugin.getResource("configurable_locales.yml")) {
            ...
            TheAnnihilation.logError("Error writing default configurable i18ns: " + exception.getMessage());
        }
        try {
            ...
            i18ns.getStringList("locales").forEach(path -> {
                ...
                if (matcher.matches()) {
                    try {
                        ...
                        TheAnnihilation.logError("Error loading locale: " + exception.getMessage());
                    }
                } else {
                    TheAnnihilation.logError("Error loading locale '" + path + "': Could not match locale");
                }
            });
        } catch (NullPointerException | YAMLException exception) {
            TheAnnihilation.logError("Error loading locales: " + exception.getMessage());
        }
    }
    public static void registerTranslation(Locale locale, String key, Component translation) {
        ...
        TATranslator.registerTranslation(locale, key, translation);
    }
