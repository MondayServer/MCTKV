...
final class MultiverseWorldHost implements WorldHost {
    ...
    public Optional<World> load() {
        worldManager.getWorld(world).peek(world -> worldManager.deleteWorld(DeleteWorldOptions.world(world)).getOrNull());
    ...
    public void tryAddEntities(List<Entity> entities, int delay) {
        try {
            ...
            TheAnnihilation.logError("Failed to add entities to " + world + "! Retrying in " + delay + " ticks");
            Bukkit.getScheduler().runTaskLater(TheAnnihilation.getInstance(), () -> tryAddEntities(entities, delay * 2 + EXPONENTIAL_BACKUP_RANDOM.nextInt(2)), delay);
        }
    }
    @Override
    public void unload() {
    ...
    public void tryUnload(int delay) {
        multiverseWorld.getPlayers().fold(AsyncAttemptsAggregate::emptySuccess, players -> safetyTeleporter.to(Utils.HUB).checkSafety(false).teleport(players)).onSuccess(() -> worldManager.deleteWorld(DeleteWorldOptions.world(multiverseWorld)).onFailure(() -> {
            TheAnnihilation.logError("Failed to delete world " + world + "! Retrying in " + delay + " ticks");
            Bukkit.getScheduler().runTaskLater(TheAnnihilation.getInstance(), () -> tryUnload(delay * 2 + EXPONENTIAL_BACKUP_RANDOM.nextInt(2)), delay);
        })).onFailure(() -> {
            TheAnnihilation.logError("Failed to remove players from " + world + "! Retrying in " + delay + " ticks");
            Bukkit.getScheduler().runTaskLater(TheAnnihilation.getInstance(), () -> tryUnload(delay * 2 + EXPONENTIAL_BACKUP_RANDOM.nextInt(2)), delay);
        });
    }
}
